...
BINDIR=$ROOT/bin
REFDIR=$ROOT/ref
FRAGDIR=$ROOT/examples/water/4-fit-frags/frag1/fit1  # folder containing combined fragment xyz files
# INPUT
...
for ((i=$MINCHGS; i<=$MAXCHGS; i++)); do
  if [ ! -e $FRAGDIR/$i"charges.xyz" ]; then
    echo "Error: file $FRAGDIR/$i charges.xyz not found!"
    exit
  else
    INITIALXYZ=$FRAGDIR/$i"charges.xyz"
  fi
